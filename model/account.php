...
class SysAdmin{
    ...
    public function createSysAdminAccount($profile) {
        ...
        try {
            error_log("Attempting to create SysAdmin account...");
            $stmt = $conn->prepare("INSERT INTO sysadmin (userName, name, email, password, profile) VALUES (?, ?, ?, ?, ?)");
            if (!$stmt) {
                throw new Exception("Prepare statement failed: " . $conn->error);
            }

            $stmt->bind_param("sssss", $this->userName, $this->name, $this->email, $this->password, $profile);
...
class BusinessOwner{
    ...
    public function createBusinessOwnerAccount($profile) {
        ...
        try {
            error_log("Attempting to create BusinessOwner account...");
            $stmt = $conn->prepare("INSERT INTO businessowner (userName, name, email, password, profile, company) VALUES (?, ?, ?, ?, ?, ?)");
            if (!$stmt) {
                throw new Exception("Prepare statement failed: " . $conn->error);
            }

            $stmt->bind_param("ssssss", $this->userName, $this->name, $this->email, $this->password, $profile, $this->company);
